...
<head>
	<title>Torneo del 18 de Enero de Luque - Listar Equipos</title>
...
function editar(id){
	window.location.assign("EquipoServlet?ACCION=EDITAR&id="+id);
}
...
<hr />
	...
	<c:choose>
		<c:when test="${param.STATUS == 'DEL_SUCCES'}">
			<div class="alert alert-success" role="alert">equipo(s) eliminado(s) exitosamente</div>
		</c:when>
		<c:when test="${param.STATUS == 'DEL_REJECT'}">
			<div class="alert alert-danger" role="alert">debe seleccionar al menos un equipo</div>
		</c:when>
	</c:choose>
	...
	<table class="table" id="tabla-opciones">
	  ...
	  <tbody>
	    ...
	    <tr>
	      ...
	      <td>${eqp.nombre}</td>
	      <td>${eqp.slogan}</td>
	      <td>${eqp.capitan.apellidos} ${eqp.capitan.nombres}</td>
	      <td align="center">
	      
<button type="button" class="btn btn-primary" onclick="editar(${eqp.id});">
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16">
<path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325"></path>
</svg>
</button>

<input type="checkbox" class="btn-check" autocomplete="off" id="multi${eqp.id}" name="equiposid" value="${eqp.id}"/>
<label class="btn btn-outline-primary" for="multi${eqp.id}"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
	  ...
	  <div class="modal-dialog">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	        Estas seguro que desea eliminar los registros seleccionados?
	      </div>
	      <div class="modal-footer">
	        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
	        <button type="button" class="btn btn-primary" onclick="eliminarEquipos();" >Aceptar</button>
	      </div>
	    </div>
	  </div>
	</div>
